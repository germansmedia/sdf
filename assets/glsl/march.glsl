...
bool march_ray(
    ...
    in vec3 dp,  // march direction
    out float r,  // distance to object
    out float attenuation,  // shadow attenuation
    out uint steps,  // how many steps were taken
    out uint iterations,  // how many iterations were needed
...
float measure_depth(in vec3 p,in vec3 dp,in float sr_per_pixel) {
    ...
    uint steps;
    uint iterations;
    if(march_ray(p,dp,r,attenuation,steps,iterations,sr_per_pixel,uniforms.params.horizon)) {
        return r;
    }
    else {
...
float process_shadow(in vec3 p,in vec3 dp,in float sr_per_pixel,in float r_max) {
    ...
    uint steps;
    uint iterations;
    if(march_ray(p,dp,r,attenuation,steps,iterations,sr_per_pixel,r_max)) {
        return 0.0;
    }
    else {
        return clamp(uniforms.params.shadow_power.a * attenuation,0.0,1.0);
    }
}
